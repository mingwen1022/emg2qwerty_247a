...
def main(config: DictConfig):
    ...
    def _full_session_paths(
        ...
        if max_sessions is not None and max_sessions < len(paths):
    ...
    loggers = True  # Lightning default
    if logger_configs:
        loggers = [instantiate(cfg) for cfg in logger_configs]
    ...
    # Write results to JSON for collect_ablation_results.py (Hydra cwd is run dir)
    import json
